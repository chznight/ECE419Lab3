...
public class MazeImpl extends Maze implements Serializable, ClientListener, Runnable {
        ...
        public synchronized void addClient_with_point(Client client, Point point, int direction) {
                assert(client != null);
                // Pick a random starting point, and check to see if it is already occupied
                CellImpl cell = getCellImpl(point);
                // Repeat until we find an empty cell
                if (cell.getContents() != null) {
                        System.out.println ("Point already taken server error?");
                } 
                addClient_with_point_direction(client, point, direction);
        }

        private synchronized void addClient_with_point_direction(Client client, Point point, int direction) {
                assert(client != null);
                assert(checkBounds(point));
                CellImpl cell = getCellImpl(point);
                Direction d = Direction.new_direction(direction);
                while(cell.isWall(d)) {
                   d = Direction.new_direction((++direction)%4);
                }
